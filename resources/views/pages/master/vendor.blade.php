...
@section('container')
    <div class="p-6 bg-white rounded-lg shadow-lg">
        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden mt-6">
            <table class="min-w-full text-sm text-center">
                ...
                <tbody id="vendor-data" class="text-gray-700">
                    @foreach ($vendors as $number => $item)
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            <td class="py-3 px-6">
                                {{ $loop->iteration + ($vendors->currentPage() - 1) * $vendors->perPage() }}</td>
                            <td class="py-3 px-6 text-left">{{ $item->name }}</td>
                            <td class="py-3 px-6">{{ $item->phone }}</td>
                            <td class="py-3 px-6 text-left">{{ $item->address }}</td>
                            <form id="delete-form-{{ $item->id }}"
                                action="{{ route('master.vendor.destroy', $item->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                            </form>
    ...
    <script>
        ...
        Input.addEventListener('input', function() {
            ...
            timeout = setTimeout(() => {
                if (query.length > 0) {
                    document.getElementById('vendor-data').classList.add('hidden')
                    document.getElementById('vendor-value').classList.remove('hidden')
                    fetch(`/vendor-search?query=${query}`)
                        .then(response => response.json())
                        .then(data => {
                            const suggestions = document.getElementById('vendor-value');
                    ...
                    document.getElementById('vendor-data').classList.remove('hidden')
                    document.getElementById('vendor-value').classList.add('hidden')
                }
            }, 400);
        });
