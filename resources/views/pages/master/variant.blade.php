    ...
    <div class="flex justify-between mb-4">
        <div class="w-1/2">
            <form action="{{ route('master.variant.store') }}" method="POST">
                @csrf
                <input type="text" name="name" class="border border-gray-300 rounded p-2 w-3/4" placeholder="Inputkan Jenis obat">
                ...
                @foreach ($variants as $number=>$item)
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        ...
                        <td class="py-3 px-6 text-center">
                            ...
                            <form action="{{ route('master.variant.destroy',$item->id) }}" class="inline" method="POST">
                                @csrf
                                @method('DELETE')
