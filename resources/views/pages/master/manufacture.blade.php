...
@section('container')
            ...
            <div class="w-1/2">
                <form id="create-manufacture-form" action="{{ route('master.manufacture.store') }}" method="POST">
                    ...
                    <input type="text" name="name" class="border border-gray-300 rounded-lg p-2 w-3/4"
                        placeholder="Inputkan Produsen obat">
                    <button type="submit"
                        class="bg-blue-500 text-white rounded-lg hover:bg-blue-600 px-6 py-2  ">Submit</button>
                </form>
            </div>
                ...
                <tbody class="text-gray-700">
                    @foreach ($manufactures as $number => $item)
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            <td class="py-3 px-6">
                                {{ $loop->iteration + ($manufactures->currentPage() - 1) * $manufactures->perPage() }}</td>
    ...
    <div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden">
        <div class="bg-white rounded-lg shadow-lg p-8 w-96">
            <h2 class="text-center text-xl font-semibold mb-6">Edit Produsen</h2>
