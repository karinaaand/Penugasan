    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <table class="min-w-full text-sm text-center">
            ...
            <tbody class=">
                ...
                <td class="py-3 px-6">{{ $loop->iteration + ($categories->currentPage() - 1) * $categories->perPage() }}</td>
                <td class="py-3 px-6 w-6">{{ $item->code }}</td>
                <td class="py-3 px-6 text-left">{{ $item->name }}</td>
                <form id="delete-form-{{ $item->id }}" action="{{ route('master.category.destroy',$item->id) }}" method="post">
                    @csrf
                    @method('DELETE')
                </form>
