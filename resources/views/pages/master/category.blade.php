...
@section('container')
                    ...
                    @foreach ($categories as $number => $item)
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            ...
                            <form id="delete-form-{{ $item->id }}"
                                action="{{ route('master.category.destroy', $item->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                            </form>
                            <td class="py-3 px-6 flex justify-center">
                                <a onclick="showEditModal('{{ $item->name }}','{{ $item->code }}','{{ $item->id }}')"
                                    class="flex cursor-pointer items-center bg-yellow-300 text-white text-sm px-2 py-2 rounded-lg shadow hover:bg-yellow-400 mr-2">
    ...
    <script>
        ...
        categoryInput.addEventListener('input', function() {
            ...
            timeout = setTimeout(() => {
                if (query.length > 0) {
                    document.getElementById('category-data').classList.add('hidden')
                    document.getElementById('category-value').classList.remove('hidden')
                    fetch(`/category-search?query=${query}`)
                        .then(response => response.json())
                        .then(data => {
                            const suggestions = document.getElementById('category-value');
                    ...
                    document.getElementById('category-value').classList.add('hidden')
                }
            }, 400);
        });
