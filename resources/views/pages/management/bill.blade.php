        ...
        <table class="min-w-full leading-normal">
            ...
            <tbody class="text-gray-600 text-sm font-light">
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    <td class="py-3 px-6 text-center text-base font-inter">{{ $number+1 }}</td>
                    <td class="py-3 px-6 text-center text-base font-inter">{{ $item->transaction()->code }}</td>
                    <td class="py-3 px-6 text-center text-base font-inter">{{ Carbon::parse($item->arrive)->translatedFormat('j F Y') }}</td>
                    <td class="py-3 px-6 text-center text-base font-inter">{{ Carbon::parse($item->due)->translatedFormat('j F Y') }}</td>
                    <td class="py-3 px-6 text-center text-base font-inter">{{ 'Rp ' . number_format($item->total, 0, ',', '.') }}</td>
                    ...
                    @else
                    <td class="py-3 px-6 text-center text-base font-inter">
                        <span class="bg-green-500 text-base font-inter text-white py-1 px-3 text-left rounded-full">Done</span>
                    </td>
                    @endif
                    <td class="flex justify-center items-center py-3">
                        <a href="{{ route('management.bill.show', $item->id) }}" class="bg-indigo-200 hover:bg-indigo-500 p-1 rounded-md flex justify-center items-center">
