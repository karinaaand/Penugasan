    ...
    <table class="w-full text-sm text-center border border-gray-300 rounded-lg p-4 overflow-hidden shadow-lg">
        ...
        <tbody>
            ...
            <tr class="border-b" >
                ...
                <td class="py-3">{{ Carbon::parse($item->created_at)->translatedFormat('j F Y') }}</td>
                <td class="py-3">{{ Carbon::parse($item->created_at)->translatedFormat('h:i') }}</td>
                <td class="flex justify-center py-3"><a href="{{ route('transaction.show',$item->id) }}" class="bg-blue-500 hover:bg-blue-600 p-2 rounded-md"><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
